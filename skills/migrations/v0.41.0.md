...
1. **Migration v89** — partial expression index on `sources.config->>'github_repo'` for fast webhook source-lookup
...
Per-source lock + migration v89 stay on regardless (those are correctness fixes, not features).
